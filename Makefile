...
.PHONY: all clean doc src lib examples dist demojar
...
dist:	clean idl parsers batch srcarchive examplesarchive demojar docarchive binarchive

demojar: examples
	cd $(CLSDIR); jar cvfm ../demo/MeetingScheduler.jar ../demo/demo.mf demo
...
binarchive: $(CLSDIR) $(DOCDIR) all lib demojar
	cd $(ROOTDIR)/..; \
	$(ZIP) $(ZIPFLAGS) $(PACKAGE)-bin-$(VERSION).$(ZIPEXT) $(COMMON_FILES) jade/lib jade/src/starlight jade/demo; \
	cd $(ROOTDIR)
...
batch: $(CLSDIR) $(DOCDIR)
	...
	echo 'cd ..' >> $(BATCH_DIST)
	echo '$(ZIP) $(ZIPFLAGS) $(PACKAGE)-src-$(VERSION).$(ZIPEXT) $(subst /,\,$(COMMON_FILES)) jade\*.bat jade\Makefile jade\src\Makefile jade\src\*.idl jade\src\FIPA jade\src\jade jade\src\starlight jade\src\*.html jade\classes\*.mf jade\lib\Makefile jade\lib\modules\README' >> $(BATCH_DIST)
...
# otherwise an error is generated by makejade because the classes directory does not exist
	...
	echo 'cd ..' >> $(BATCH_DIST)
	echo '$(ZIP) $(ZIPFLAGS) $(PACKAGE)-examples-$(VERSION).$(ZIPEXT) $(subst /,\,$(COMMON_FILES)) jade\src\examples jade\src\demo jade\demo' >> $(BATCH_DIST)
	echo 'cd jade' >> $(BATCH_DIST)
	...
	echo 'call $(subst /,\,$(subst $(ROOTDIR),.,$(BATCH_EXAMPLES)))' >> $(BATCH_DIST) 
	echo 'cd classes' >> $(BATCH_DIST)
	echo 'jar cvfm ..\demo\MeetingScheduler.jar ..\demo\demo.mf demo' >> $(BATCH_DIST)
	echo 'cd ..\..' >> $(BATCH_DIST)
	echo '$(ZIP) $(ZIPFLAGS) $(PACKAGE)-bin-$(VERSION).$(ZIPEXT) $(subst /,\,$(COMMON_FILES)) jade\lib jade\src\starlight jade\demo' >> $(BATCH_DIST)

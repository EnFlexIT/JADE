...
batch: $(CLSDIR) $(DOCDIR)
	...
	echo 'cd ..' >> $(BATCH_DIST)
	echo '$(ZIP) $(ZIPFLAGS) $(PACKAGE)-src-$(VERSION).$(ZIPEXT) $(subst /,\,$(COMMON_FILES)) jade\*.bat jade\Makefile jade\src\Makefile jade\src\*.idl jade\src\jade jade\src\starlight jade\src\*.java jade\src\*.html jade\classes\dummyagent.gif jade\lib\Makefile' >> $(BATCH_DIST)
# jade\classes\dummyagent.gif is necessary such that the system creates the directory classes

...
class FunctorSchema implements Cloneable, Serializable {
  ...
  public FunctorSchema(Ontology o, String n, int kind) {
    myOntology = o;
    functorKind = kind;
    myName = new Name(n);
    terms = new ArrayList();
  ...
  public boolean isConcept() {
    return functorKind == CONCEPT;
  }

  public boolean isAction() {
    return functorKind == ACTION;
  }

  public boolean isPredicate() {
    return functorKind == PREDICATE;
  }

  public void checkAgainst(Functor f) throws OntologyException {
    for(int i = 0; i < terms.size(); i++) {
      TermDescriptor td = (TermDescriptor)terms.get(i);
      String name = td.getName();
      if(!td.isOptional()) {
	Object o = f.getTerm(name);
	switch(td.getType()) {
	...
	case Ontology.ACTION_TYPE: {
	  if(!(o instanceof Action))
	    throw new WrongTermTypeException(f.getName(), name, Ontology.typeNames[Ontology.ACTION_TYPE]); 
	  break;
	}
	case Ontology.PREDICATE_TYPE: {
	  if(!(o instanceof Predicate))
	    throw new WrongTermTypeException(f.getName(), name, Ontology.typeNames[Ontology.PREDICATE_TYPE]); 
	  break;
	}
	default:

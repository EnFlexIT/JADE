...
public class FIPARequestInitiator extends FSMBehaviour {
	
	// Private data store keys
	public static final String REQUEST_KEY = "__request";
	public static final String ALL_REQUESTS_KEY = "__all-requests";
	public static final String REPLY_KEY = "__reply";
	public static final String ALL_RESPONSES_KEY = "__all-responses";
	public static final String ALL_RESULT_NOTIFICATIONS_KEY = "__all-result-notifications";
	...
	/**
	   Constructs a <code>FIPARequestInitiator</code> behaviour
	   @param a The agent performing the protocol
	   @param req The request message used to initiate the protocol.
	   The default implementation of the prepareRequests() method returns
	   a <code>List</code> including that message only.
	   @param s The <code>DataStore</code> that will be used by this 
	   <code>FIPARequestInitiator</code>
	 */
	public FIPARequestInitiator(Agent a, ACLMessage req, DataStore store) {
		super(a);
		...
		getDataStore().put(REQUEST_KEY, req);
			...
			public void action() {
				...
				while(it.hasNext()) {
					...
					if (request != null) {
						...
						// FIXME: The keys for sessions should be reply-with instead of the 
						// AID 
						Iterator receivers = request.getAllReceiver();
						while (receivers.hasNext()) {
							AID r = (AID) receivers.next();
							if (!sessions.containsKey(r)) {
								sessions.put(r, new Session());
							}
						}
				...
				// to accept replies
				// FIXME: Should set all protocol fields
				MessageTemplate mt = MessageTemplate.MatchProtocol("FIPA-Request");
				rec.setMessageTemplate(mt);
				if (minTimeout > 0) {
					rec.setTimeout(minTimeout);
				}
			}	
		...
		// RECEIVE_REPLY
		rec = new MsgReceiver(myAgent, null, 0, REPLY_KEY);
		...
		b = new OneShotBehaviour(myAgent) {
			...
			public void action() {
				...
				Session s = (Session) sessions.get(sender);
				if (s != null) {

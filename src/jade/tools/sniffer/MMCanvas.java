...
public class MMCanvas extends JPanel implements MouseListener, Serializable {
  ...
  public MMCanvas(boolean type, SnifferGUI theGui, JPanel thePanel){
	...
	try{
	  xSource = al.getPos(mess.getSender().getName());
	  for (Iterator i=mess.getAllReceiver(); i.hasNext(); ) {
	    xDest = al.getPos(((AID)i.next()).getName());
	    // getPos returns 0 if the agent has not been found
	    if (xDest != 0) // if !=0 then the arrow can be painted.
	      break;
	  }
	...
	public void mousePressed(MouseEvent evt){
	  ...
	  Message mess;
   	...
   	if( ((mess = selMessage(evt)) != null) && (typeCanv == false)){
			...
			info = "Message Type: "+ ACLMessage.getPerformative(mess.getPerformative());
  ...
  public Message selMessage(MouseEvent evt){
		...
		for(Enumeration e=ml.messageVector.elements() ; e.hasMoreElements();){   
			...
			try{
				x1 = mess.getInitSeg(al.getPos(mess.getSender().getName()));
			  x2 = mess.getEndSeg(al.getPos(((AID)mess.getAllReceiver().next()).getName()));

...
public class Agent implements Runnable, Serializable, TimerListener {
  ...
  private class AgentDeathError extends Error {
     ...
     */
    AgentDeathError() {
      super("Agent "+Thread.currentThread()+" has been terminated.");
    }
   ...
   * AgentInMotionError() {
   * super("Agent "+Thread.currentThread()+" is about to move or be cloned.");
   * }
   * }
   */
  ...
  private static final AgentState[] STATES = new AgentState[] {
    new AgentState("Illegal MIN state"), new AgentState("Initiated"), new AgentState("Active"), new AgentState("Idle"), new AgentState("Suspended"), new AgentState("Waiting"), new AgentState("Deleted"), new AgentState("Transit"), new AgentState("Copy"), new AgentState("Gone"), new AgentState("Illegal MAX state")
  };
  ...
  */
  ...
  /**
   * The Home Agent Platform of this agent
   */
  private String                    myHap = null;

  /**
   * Used to make state transitions atomic
   */
  private transient Object          stateLock = new Object();

  /**
   * Used for agent waiting
   */
  private transient Object          waitLock = new Object();

  /**
   * Used for agent suspension
   */
  ...
  /**
   * The singleton TimerDispatcher in this JVM
   */
  ...
  /**
   * The internal Scheduler managing Behaviour-s
   */
  ...
  /**
   * Temporary buffer to save agent state during for agent suspension
   */
  ...
  /**
   * The arguments passed to this agent at activation time
   */
  ...
  /**
   * Return the array of arguments as they were set at activation time.
  ...
  /**
   * This method is used to simulate a call to interrupt()
   * (not available in CLDC) on the agent thread.
   * The agent thread can be in one of the following four states:
   * 1) Running a behaviour.
   * 2) Sleeping on waitLock due to a doWait()
  ...
  private synchronized void interruptThread() {
    ...
    // case 2: Signal on waitLock object.
    ...
    // case 4: Add a dummy behaviour to wake up the agent thread
    // (Scheduler.addBehaviour() notifies the scheduler monitor).
  ...
  /**
   * Check if the simulated Thread.interrupt() has been called and
   * throws InterruptedException in this case
   */
  private synchronized void checkInterrupted() throws InterruptedException {
    if (isInterrupted) {
      isInterrupted = false;
  ...
  public void doStart(String name) {
     ...
     */
    if (myToolkit != null) {
      // Note that InternalError does not exist in CLDC
  ...
  public final void run() {
    ...
    try {
      ...
      case AP_ACTIVE:
        ...
        // else
        /*AMSService.register(this, getAMS(), amsd);*/
        Runtime.instance().gc("Agent registered");
  ...
  private void mainLoop() throws InterruptedException, InterruptedIOException {
    while (myAPState != AP_DELETED) {
      ...
      case AP_ACTIVE:
        ...
        // Someone interrupted the agent. It should be a kill request...
        catch (InterruptedException ie) {
          switch (myAPState) {
        ...
        if (currentBehaviour.done()) {
          ...
          synchronized (myScheduler) {
            // See J2se version for the reason of this synchronization
  ...
  private void destroy() {
    /*try {
      ...
      AMSAgentDescription amsd = new AMSAgentDescription();
      amsd.setName(getAID());
      // System.out.println("Deregistering with AMS");
      // System.out.flush();
      AMSService.deregister(this, getAMS(), amsd);
      // System.out.println("Deregisteration complete");
      // System.out.flush();
    } 
    ...
    Runtime.instance().gc("Agent exited");
  } 
  ...
  public final ACLMessage receive() {
    synchronized (waitLock) {
      if (msgQueue.isEmpty()) {
        ...
        try {
          myToolkit.handleReceived(myAID, currentMessage);
        } 
        catch (AuthException ae) {
        } 
  ...
  public final ACLMessage blockingReceive(MessageTemplate pattern, long millis) {
    ...
    synchronized (waitLock) {
      ...
      // equals to AP_DELETED. If this is the case the following loop
      // would last forever as doWait() does nothing -->the monitor
      // of the waitLock is not released --> even if a message arrive
      // the postMessage method can't be executed.
      // Throwing AgentDeathError is necessary to exit this infinite
      // loop.
      while (msg == null) {
        long startTime = System.currentTimeMillis();
        if (myAPState == AP_ACTIVE) {
  ...
  public final void postMessage(ACLMessage msg) {
    ...
    // System.out.println("Message received" + msg.getPerformative());
    // System.out.flush();
    synchronized (waitLock) {
      if (msg != null) {
				// Handling appens first so, if an exception
      ...
      // System.out.println("Message posted. Wake agent");
      // System.out.flush();
      doWake();
    } 

    // System.out.println("Waked all threads waiting on stateLock");
    // System.out.flush();
  } 
